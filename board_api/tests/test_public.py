...
contract (status codes, cookies, error handling) in isolation. Patch targets
are board_api.routes_public.* since the three former route files are merged.
"""
...
from board_api import sessions
...
def test_signup_sets_cookie_and_returns_partner(client):
    with patch("board_api.routes_public.db.get_or_create_partner",
...
def test_me_returns_partner_with_session(client):
    ...
    with patch("board_api.routes_public.db.get_partner",
...
def test_me_401_when_partner_gone(client):
    ...
    with patch("board_api.routes_public.db.get_partner", return_value=None):
        r = client.get("/api/me")
    assert r.status_code == 401
...
def test_response_created_201(client):
    ...
    with patch("board_api.routes_public.db.get_use_case",
         ...
         patch("board_api.routes_public.db.get_partner",
