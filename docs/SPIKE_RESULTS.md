...
**Date:** 2026-07-10 · **Workspace:** an FE-VM serverless workspace (AWS us-east-1)
...
| Pinned pip installs under lockdown | ✅ GO | Installed via the internal PyPI proxy. |
| Pinned npm install + vite build under lockdown | ✅ GO | Internal npm registry serves packages; SPA builds. |
...
to disk or committed. The scratch SP and its instance role remain and should be
removed when convenient (see cleanup list below).
...
- Connecting from this laptop (external to Databricks = valid Cloud Run stand-in)
  to an existing Lakebase instance's Postgres endpoint (port 5432, `sslmode=require`)
  returned `SELECT 1` and `PostgreSQL 16.14`.
...
- **A scratch service principal** (`partner-portal-backend`) + its OAuth secret —
  created only for this spike.
- **An instance role** for that SP on the existing Lakebase instance used for the probe.

These were made against a convenient existing instance purely to prove the path.
For the real pilot, create a dedicated instance/SP and remove these. The OAuth
secret was kept in-shell only and never written to disk or git.
